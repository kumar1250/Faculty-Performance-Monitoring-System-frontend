...
export default function PatentModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  // Securely request pre-signed AWS S3 asset viewer URLs
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('patent_number', formData.patent_number);
  ...
  if (records.length > 0 && records[0].user) {
    ...
    try {
      if (editingId) {
        ...
        // Hits POST /patents/patent/ (With trailing slash)
        await API.post('/patents/patent/', data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      setIsOpen(false);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Patents Registered & Published</h3>
          <p className="text-xs text-slate-500 mt-0.5">Manage intellectual property portfolios, including published ideas and granted innovations.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No intellectual property or patent filings registered.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Serial Code: <strong className="text-slate-700 font-mono text-[11px]">{record.patent_number}</strong></span>
                  <span>Classification: <strong className="text-slate-700 capitalize">{record.patent_type?.toLowerCase().replace('_', ' ')}</strong></span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-md">IP Property Roster View</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Patent Registration Serial</span>
                <strong className="text-slate-800 text-base mt-0.5 block font-mono">{selectedRecord.patent_number}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Filing Category Allocation</span>
                <strong className="text-slate-800 text-base mt-0.5 block capitalize">{selectedRecord.patent_type?.toLowerCase().replace('_', ' ')}</strong>
              </div>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Uploaded Office Gazette Certificate Proof</label>
              {selectedRecord.certificate_file ? (
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-900 h-[280px] flex flex-col justify-center items-center relative group/file">
                  {secureFileUrl ? (
                    <iframe src={secureFileUrl} className="w-full h-full border-0 bg-white" title="Patent Copy Gazette Preview" />
                  ) : (
                ...
                <div className="border border-dashed border-slate-200 bg-slate-50 text-slate-400 italic text-xs py-8 rounded-xl text-center">No structural validation asset linked onto this entry structure.</div>
              )}
            </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Intellectual Property' : 'Register New Patent Tracking Line'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Patent / Innovation Title</label>
                  <input type="text" name="title" required value={formData.title} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Official Patent Filing / Grant Number</label>
                  <input type="text" name="patent_number" required value={formData.patent_number} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" placeholder="e.g., IN-20264101" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Filing Classification Status</label>
                  <select name="patent_type" value={formData.patent_type} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="GRANTED_FIRST">GRANTED (First / Principal Applicant)</option>
                    <option value="GRANTED_OTHER">GRANTED (Co-Applicant / Joint Member)</option>
                    <option value="PUBLISHED_FIRST">PUBLISHED (First / Principal Inventor)</option>
                    <option value="PUBLISHED_OTHER">PUBLISHED (Co-Inventor Member Team)</option>
                  </select>
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Gazette Copy Proof File (.jpg, .jpeg, .png, .pdf)</label>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Line...' : 'Save Patent Entry'}
                </button>
              </div>
            </form>
          </div>
