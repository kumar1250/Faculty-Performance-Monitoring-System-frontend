...
export default function PublicationModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  const [formData, setFormData] = useState({
    ...
    indexing_type: 'IEEE_SPRINGER_ELSEVIER',
  ...
  const handleInputChange = (e) => {
    ...
    // Backend model rule constraint: Book chapters support Scopus indexing only
    if (name === 'publication_type' && value === 'BOOK_CHAPTER') {
      setFormData({
        ...formData,
        [name]: value,
  ...
  // Securely resolve pre-signed S3 verification asset links from backend
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('indexing_type', formData.indexing_type);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Research Publications</h3>
          <p className="text-xs text-slate-500 mt-0.5">Track and submit conference proceedings and book chapters with verified indexing tiers.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No research publications or conference proceedings logged.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Type: <strong className="text-slate-700 capitalize">{record.publication_type?.toLowerCase().replace('_', ' ')}</strong></span>
                  <span>Indexing: <strong className="text-slate-700 uppercase">{record.indexing_type?.replace('_', ' ')}</strong></span>
                  <span>Role: <strong className="text-slate-700 capitalize">{record.author_type?.toLowerCase().replace('_', ' ')}</strong></span>
                </div>
              </div>
