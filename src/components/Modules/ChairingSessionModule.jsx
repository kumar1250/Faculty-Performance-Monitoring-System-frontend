...
export default function ChairingSessionModule({ records = [], isReadOnly, currentRegisterNo, currentUserId, onRefresh }) {
  ...
  const handleInputChange = (e) => {
    ...
    // Rubric Warning: Chairing/Co-chairing sessions are only valid for International and National levels
    if (name === 'event_type' && value === 'CHAIRING_SESSION') {
      setFormData({
        ...formData,
        [name]: value,
        event_level: 'INTERNATIONAL'
  ...
  // Securely resolve pre-signed S3 download URLs[cite: 25]
  useEffect(() => {
    async function fetchSecureAsset() {
      ...
      try {
        // Hits backend: GET /session/chairing/<id>/file
        const response = await API.get(`/session/chairing/${selectedRecord.id}/file`);
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('event_level', formData.event_level);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Sessions Chaired & Invited Lectures</h3>
          <p className="text-xs text-slate-500 mt-0.5">Track keynote speeches, guest lectures, and conference sessions chaired externally.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No chaired sessions or invited talks logged under this portfolio.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{record.event_name}</p>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Role Type: <strong className="text-slate-700 capitalize">{record.event_type?.toLowerCase().replace('_', ' ')}</strong></span>
                  <span>Level: <strong className="text-slate-700 uppercase">{record.event_level?.replace('_', '/')}</strong></span>
                  <span>At: <strong className="text-slate-700">{record.organization}</strong></span>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                ...
                <h4 className="text-xl font-black text-slate-900 mt-1">{selectedRecord.event_name}</h4>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Activity Engagement Track</span>
                <strong className="text-slate-800 text-base mt-0.5 block capitalize">{selectedRecord.event_type?.toLowerCase().replace('_', ' ')}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Event Scope Level Tier</span>
                <strong className="text-slate-800 text-base mt-0.5 block uppercase">{selectedRecord.event_level?.replace('_', '/')}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Host Organization Entity</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.organization}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Execution Timeline Date</span>
                <strong className="text-slate-800 text-base mt-0.5 block font-mono">{selectedRecord.event_date}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex justify-between items-center sm:col-span-2">
                <div>
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Appraisal Verification Status</span>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Uploaded Certificate / Invitation Letter Proof</label>
              {selectedRecord.certificate_file ? (
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-900 h-[280px] flex flex-col justify-center items-center relative group/file">
                  {secureFileUrl ? (
                    <iframe src={secureFileUrl} className="w-full h-full border-0 bg-white" title="Proof Preview" />
                  ) : (
                ...
                <div className="border border-dashed border-slate-200 bg-slate-50 text-slate-400 italic text-xs py-8 rounded-xl text-center">No structural proof asset linked onto this entry structure.</div>
              )}
            </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Session Details' : 'Register Coordinated Lecture / Session'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Event / Lecture Program Topic Title</label>
                  <input type="text" name="event_name" required value={formData.event_name} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Host Organization Entity</label>
                  <input type="text" name="organization" required value={formData.organization} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Activity Capacity Track</label>
                  <select name="event_type" value={formData.event_type} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="INVITED_TALK">Invited Talk</option>
                    <option value="GUEST_LECTURE">Guest Lecture</option>
                    <option value="KEYNOTE_SPEECH">Keynote Speech</option>
                    <option value="CHAIRING_SESSION">Session Chair / Co-Chair</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Event Scope Level Tier</label>
                  <select name="event_level" value={formData.event_level} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="INTERNATIONAL">International Level</option>
                    <option value="NATIONAL">National Level</option>
                    {formData.event_type !== 'CHAIRING_SESSION' && (
                      <>
                        <option value="IIT_NIT">IIT / NIT Hosted</option>
                        <option value="UNIVERSITY">University Level</option>
                        <option value="COLLEGE">Affiliated College Level</option>
                      </>
                    )}
                  </select>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Execution Event Date</label>
                  <input type="date" name="event_date" required value={formData.event_date} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Verification Document Proof (.jpg, .jpeg, .png, .webp)[cite: 25]</label>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Record Track...' : 'Save Activity Entry'}
                </button>
              </div>
            </form>
          </div>
