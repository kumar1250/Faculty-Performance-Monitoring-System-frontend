...
export default function StudentProjectModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  const [formData, setFormData] = useState({
    project_title: '',
    project_type: 'BTECH',
    publication_status: 'WITHOUT_PUBLICATION',
    student_names: '',
    academic_year: '',
  ...
  useEffect(() => {
    async function fetchSecureAsset() {
      ...
      try {
        const response = await API.get(`/project/studentproject/${selectedRecord.id}/file/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await API.put(`/project/studentproject/${editingId}/update/`, data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await API.post('/project/studentproject/create/', data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      setIsOpen(false);
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      await API.delete(`/project/studentproject/${id}/delete/`);
      onRefresh();
    } catch (err) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Student Project Works Undertaken</h3>
          <p className="text-xs text-slate-500 mt-0.5">Track guided B.Tech or M.Tech student group projects and publication statuses.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No student projects logged under this portfolio.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{record.project_title}</p>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Stream Tier: <strong className="text-slate-700">{record.project_type}</strong></span>
                  <span>Publication: <strong className="text-slate-700 capitalize">{record.publication_status?.toLowerCase().replace('_', ' ')}</strong></span>
                  <span>Year: <strong className="text-slate-700">{record.academic_year || 'N/A'}</strong></span>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Project Program Stream</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.project_type} Degree</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Output Publication Status</span>
                <strong className="text-slate-800 text-base mt-0.5 block capitalize">{selectedRecord.publication_status?.toLowerCase().replace('_', ' ')}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Academic Execution Year</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.academic_year || 'N/A'}</strong>
              </div>
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 sm:col-span-2">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Guided Student Team Names</span>
                <p className="text-slate-800 text-sm font-medium mt-1 whitespace-pre-wrap">{selectedRecord.student_names || 'None registered'}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Uploaded Project Approval Certificate / Report Cover Page</label>
              {selectedRecord.certificate_file ? (
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-900 h-[280px] flex flex-col justify-center items-center relative group/file">
                  {secureFileUrl ? (
                    <iframe src={secureFileUrl} className="w-full h-full border-0 bg-white" title="Project Proof Document Preview" />
                  ) : (
                    <div className="text-white text-xs flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                      <span>Resolving secure file layer stream token...</span>
                    </div>
                  )}
                  {secureFileUrl && (
                ...
                <div className="border border-dashed border-slate-200 bg-slate-50 text-slate-400 italic text-xs py-8 rounded-xl text-center">No validation asset uploaded onto this project entry record.</div>
              )}
            </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Project Entry' : 'Register Guided Student Project Work'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Project Investigation Title</label>
                  <input type="text" name="project_title" required minLength={3} value={formData.project_title} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Degree Program Track</label>
                  <select name="project_type" value={formData.project_type} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="BTECH">B.Tech Project Work</option>
                    <option value="MTECH">M.Tech Thesis Work</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Associated Output Publication</label>
                  <select name="publication_status" value={formData.publication_status} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="WITHOUT_PUBLICATION">Without Publication Outcome</option>
                    <option value="WITH_PUBLICATION">With Publication Outcome</option>
                  </select>
                </div>
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Student Group Member Names (Comma separated or multi-line)</label>
                  <textarea name="student_names" required value={formData.student_names} onChange={handleInputChange} className="w-full text-sm p-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none h-20 resize-none" placeholder="e.g., Student A, Student B, Student C" />
                </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Project Details...' : 'Save Project Record'}
                </button>
              </div>
            </form>
          </div>
