...
export default function FDPsAttendedModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  const openEditModal = (e, record) => {
    ...
    setFormData({
      ...
      duration: record.duration || 'GE_2W',
      level: record.level || 'INTERNATIONAL',
      certificate_file: null
    });
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await API.put(`/fdps-attend/fdp/${editingId}/update/`, data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await API.post('/fdps-attend/fdp/create/', data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      setIsOpen(false);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">FDPs, Workshops & Seminars Attended</h3>
          <p className="text-xs text-slate-500 mt-0.5">Manage and append upskilling validation metrics for courses attended at IIT, NIT, or international tracks.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No attended FDPs or conference event logs found.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Stream: <strong className="text-slate-700">{record.category}</strong></span>
                  <span>Host: <strong className="text-slate-700">{record.institute}</strong></span>
                  {record.category === 'FDP' ? (
                    <span>Duration: <strong className="text-slate-700">{record.duration?.replace('GE_', '>= ').replace('BW_', '').replace('LT_', '< ')}</strong></span>
                  ) : (
                    <span>Level: <strong className="text-slate-700 capitalize">{record.level?.toLowerCase()}</strong></span>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-md">Appraisal File Review</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Category Stream</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.category}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Hosting Organization</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.institute}</strong>
              </div>
              {selectedRecord.category === 'FDP' ? (
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Duration Span Scale</span>
                  <strong className="text-slate-800 text-base mt-0.5 block uppercase">{selectedRecord.duration?.replace('_', ' ')}</strong>
                </div>
              ) : (
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Conference Global Level</span>
                  <strong className="text-slate-800 text-base mt-0.5 block uppercase">{selectedRecord.level}</strong>
                </div>
              )}
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Event Record' : 'Log Attended FDP / Seminar / Conference'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Event / Course Program Name Title</label>
                  <input type="text" name="title" required value={formData.title} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" placeholder="e.g., Advanced AI Upskilling Workshop" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Event Stream Category</label>
                  <select name="category" value={formData.category} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Hosting Institute Tier</label>
                  <select name="institute" value={formData.institute} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="IIT">IIT Tier Track</option>
                    <option value="NIT">NIT Tier Track</option>
                    <option value="UNIVERSITY">University Level</option>
                    <option value="COLLEGE">Affiliated College</option>
                    <option value="ABROAD">ABROAD (International Track)</option>
                  </select>
                </div>

                {formData.category === 'FDP' ? (
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Duration Span Scale</label>
                ...
                <div className="sm:col-span-2">
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Conference Level Tier</label>
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Verification Proof Asset (.jpg, .jpeg, .png, .pdf)</label>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Event Track...' : 'Save Activity Entry'}
                </button>
              </div>
            </form>
          </div>
