...
export default function ProfessionalMembershipModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  // Securely request pre-signed S3 verification asset links from backend
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('membership_id', formData.membership_id);
  ...
  if (records.length > 0 && records[0].user) {
    ...
    try {
      if (editingId) {
        ...
        // Hits POST /professional-membership/professional/membership/ (With trailing slash)
        await API.post('/professional-membership/professional/membership/', data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
      setIsOpen(false);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Memberships with Professional Bodies</h3>
          <p className="text-xs text-slate-500 mt-0.5">Log, manage, and verify status credentials held in national or international expert communities.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No expert or professional body memberships registered.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{record.organization_name}</p>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Scope Tier: <strong className="text-slate-700 uppercase">{record.membership_type}</strong></span>
                  <span>ID Index: <strong className="text-slate-700">{record.membership_id || 'N/A'}</strong></span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-md">Membership Audit View</span>
                <h4 className="text-xl font-black text-slate-900 mt-1">{selectedRecord.organization_name}</h4>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Professional Organization Name</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.organization_name}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Membership Scope Tier</span>
                <strong className="text-slate-800 text-base mt-0.5 block uppercase">{selectedRecord.membership_type}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Membership Registry ID Serial</span>
                <strong className="text-slate-800 text-base mt-0.5 block font-mono">{selectedRecord.membership_id || 'N/A'}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Registration Effective Date</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.membership_date}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex justify-between items-center sm:col-span-2">
                <div>
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Appraisal Approval Status</span>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Uploaded Credentials Identification Image Proof</label>
              {selectedRecord.certificate_file ? (
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-900 h-[280px] flex flex-col justify-center items-center relative group/file">
                  {secureFileUrl ? (
                    <iframe src={secureFileUrl} className="w-full h-full border-0 bg-white" title="Membership Identification Document Preview" />
                  ) : (
                    <div className="text-white text-xs flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                      <span>Resolving secure file tracking link...</span>
                    </div>
                  )}
                  {secureFileUrl && (
                ...
                <div className="border border-dashed border-slate-200 bg-slate-50 text-slate-400 italic text-xs py-8 rounded-xl text-center">No structural verification asset linked onto this entry structure.</div>
              )}
            </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Membership Entry' : 'Register New Professional Membership'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Professional Body / Organization Name</label>
                  <input type="text" name="organization_name" required value={formData.organization_name} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" placeholder="e.g., Association for Computing Machinery (ACM)" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Membership ID Reference Number</label>
                  <input type="text" name="membership_id" value={formData.membership_id} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" placeholder="e.g., ACM-2481026" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Scope Jurisdiction Tier</label>
                  <select name="membership_type" value={formData.membership_type} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="NATIONAL">NATIONAL Body (e.g., ISTE, CSI)</option>
                    <option value="INTERNATIONAL">INTERNATIONAL Body (e.g., IEEE, ACM)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Registration Date</label>
                  <input type="date" name="membership_date" required value={formData.membership_date} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Verification Proof Card/Certificate (.jpg, .jpeg, .png, .webp)</label>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Data...' : 'Save Membership Record'}
                </button>
              </div>
            </form>
          </div>
