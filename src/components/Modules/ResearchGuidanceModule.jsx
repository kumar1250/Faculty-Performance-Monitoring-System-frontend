...
export default function ResearchGuidanceModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  const openEditModal = (e, record) => {
    ...
    setFormData({
      scholar_name: record.scholar_name,
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      scholar_name: formData.scholar_name,
      guide_type: formData.guide_type,
      registration_date: formData.registration_date,
      status: formData.status,
      awarded_date: formData.status === 'awarded' ? formData.awarded_date : null
    };
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Research Guidance (Ph.D / M.Phil Supervision)</h3>
          <p className="text-xs text-slate-500 mt-0.5">Manage and view degree guidance paths, active supervision, and awarded thesis points tracking panels.</p>
        </div>
        {!isReadOnly && (
          <button onClick={openCreateModal} className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition">
            + Log Scholar Guidance
          </button>
        )}
      </div>

      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No research guidance tracks logged under this portfolio.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{record.scholar_name}</p>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Role Capacity: <strong className="text-slate-700">{record.guide_type}</strong></span>
                  <span>Status Track: <strong className="text-slate-700 capitalize">{record.status}</strong></span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-md">Supervision Inspection View</span>
                <h4 className="text-xl font-black text-slate-900 mt-1">{selectedRecord.scholar_name}</h4>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Guide Assignment Level</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.guide_type}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Filing Operational Status</span>
                <strong className="text-slate-800 text-base mt-0.5 block capitalize">{selectedRecord.status}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Initial Joining Registration Date</span>
                <strong className="text-slate-800 text-base mt-0.5 block font-mono">{selectedRecord.registration_date}</strong>
              </div>
              {selectedRecord.status === 'awarded' && (
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Degree Conferral Awarded Date</span>
                  <strong className="text-slate-800 text-base mt-0.5 block font-mono">{selectedRecord.awarded_date}</strong>
                </div>
              )}
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Guidance Track' : 'Register New Student Supervision'}</h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Scholar Student Name</label>
                  <input type="text" name="scholar_name" required minLength={3} value={formData.scholar_name} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Supervision Capacity Role</label>
                  <select name="guide_type" value={formData.guide_type} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="Guide">Primary Guide / Main Supervisor</option>
                    <option value="Co-Guide">Co-Guide / Joint Supervisor</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Research Supervision Status</label>
                  <select name="status" value={formData.status} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 bg-slate-50/50 rounded-xl focus:bg-white font-medium text-slate-700">
                    <option value="ongoing">Ongoing Supervision Track</option>
                    <option value="awarded">Degree Conferred / Awarded</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Initial Registration Date</label>
                  <input type="date" name="registration_date" required value={formData.registration_date} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                </div>

                {formData.status === 'awarded' && (
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">Thesis Awarded Date</label>
                    <input type="date" name="awarded_date" required value={formData.awarded_date} onChange={handleInputChange} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl bg-slate-50/50 focus:bg-white focus:outline-none" />
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Track Line...' : 'Save Guidance Entry'}
                </button>
              </div>
            </form>
          </div>
