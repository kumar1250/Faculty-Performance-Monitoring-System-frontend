...
export default function CourseCertifications({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  // UX State: Track selected record for detailed viewing drawer
  ...
  // Securely fetch pre-signed S3 URLs using your authorization interceptor
  useEffect(() => {
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      ...
      alert('Failed to delete course certification entry.');
    }
  };
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Online Courses & Certifications</h3>
          <p className="text-xs text-slate-500 mt-0.5">Manage professional upskilling validation records like NPTEL courses.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No completed course achievements or certifications logged.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors">{record.Course_name}</p>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Type Evaluation Stream: <strong className="text-slate-700 uppercase">{record.certificate_type}</strong></span>
                </div>
              </div>
