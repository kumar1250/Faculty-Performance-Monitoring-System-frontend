...
export default function FundedProjectsModule({ records = [], isReadOnly, currentUserId, onRefresh }) {
  ...
  const [formData, setFormData] = useState({
    ...
    funding_agency: '',
    grant_amount: '',
    grant_category: 'gt_10',
    investigator_role: 'pi',
    sanction_date: '',
    completion_date: ''
  });
  ...
  const openEditModal = (e, record) => {
    ...
    setFormData({
      ...
      grant_category: record.grant_category,
      investigator_role: record.investigator_role,
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      grant_category: formData.grant_category,
      investigator_role: formData.investigator_role,
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6 space-y-4 font-sans mt-6">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Funded Research Projects</h3>
          <p className="text-xs text-slate-500 mt-0.5">Manage corporate and government institutional grant-funded research operations data.</p>
        </div>
        {!isReadOnly && (
      ...
      {records.length === 0 ? (
        <p className="text-slate-400 text-sm italic py-2">No funded institutional research project logs posted.</p>
      ) : (
            ...
            >
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 font-medium">
                  <span>Agency: <strong className="text-slate-700">{record.funding_agency}</strong></span>
                  <span>Grant: <strong className="text-slate-700">₹{parseFloat(record.grant_amount).toLocaleString()}</strong></span>
                  <span>Role: <strong className="text-slate-700 uppercase">{record.investigator_role?.replace('_', ' ')}</strong></span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full p-6 space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <span className="text-[10px] font-black tracking-wider uppercase px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-md">Research Project Audit View</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b border-slate-100 pb-5">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Sponsoring Funding Agency</span>
                <strong className="text-slate-800 text-base mt-0.5 block">{selectedRecord.funding_agency}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Total Sanctioned Grant Fund</span>
                <strong className="text-slate-800 text-base mt-0.5 block">₹{parseFloat(selectedRecord.grant_amount).toLocaleString()}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Investigator Ownership Capacity</span>
                <strong className="text-slate-800 text-base mt-0.5 block uppercase">{selectedRecord.investigator_role?.replace('_', ' ')}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Sanction / Completion Timeline</span>
                <strong className="text-slate-800 text-sm mt-0.5 block">{selectedRecord.sanction_date} to {selectedRecord.completion_date || 'Ongoing'}</strong>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex justify-between items-center sm:col-span-2">
                <div>
                  <span className="text-xs font-bold text-slate-400 block uppercase tracking-wide">Evaluation Status</span>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <h4 className="text-lg font-bold text-slate-900">{editingId ? 'Modify Project Data' : 'Log New Funded Research Grant'}</h4>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition disabled:opacity-50">
                  {loading ? 'Saving Project Track...' : 'Save Activity Entry'}
                </button>
              </div>
            </form>
          </div>
