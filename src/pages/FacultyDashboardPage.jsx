...
export default function FacultyDetailModal({ data, onClose }) {
  ...
  const [department, setDepartment] = useState(data?.department ?? null);
  ...
  useEffect(() => {
    ...
    if (!data?.register_no) return;
    let cancelled = false;
    getFacultyDirectory().then((directory) => {
      if (!cancelled) setDepartment(directory[data.register_no] ?? null);
    });
    return () => { cancelled = true; };
  }, [data?.department, data?.register_no]);
  ...
  return (
    <>
      {activeModule && (
        <ModuleDrawer module={activeModule} department={department} onClose={() => setActiveModule(null)} />
      )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-1.5 text-sm text-blue-100">
                    ...
                    {departmentLabel && <span>🏛️ {departmentLabel}</span>}
                  </div>
                </div>
              </div>
