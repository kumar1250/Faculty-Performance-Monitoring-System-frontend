  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border shadow-sm">
        <h2 className="text-2xl font-black text-slate-900">Faculty: {data.username}</h2>
        <p className="text-slate-500">Register No: {data.register_no}</p>
      </div>

      {/* Render your data structure here */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {data.modules.map((mod, index) => (
          <div key={index} className="bg-white p-5 rounded-2xl border shadow-sm">
            <h3 className="font-bold text-slate-800">{mod.module}</h3>
